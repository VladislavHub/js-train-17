...
function checkWord(word, text) {
  ...
  const wordTest = searchWord.test(text)
  return wordTest
}
...
function countEmails(str) {
  ...
  const searchEmail = /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/g

  const matches = str.match(searchEmail)
...
function checkRegexFlags(regex) {
  ...
  const hasGlobalAFlag = flags.includes('g')
  const hasMultilineFlag = flags.includes('m')

  return hasGlobalAFlag && hasMultilineFlag

  // const check = new RegExp(regex)
  // if(check.global === true && check.multiline === true){
  //   return true
  // }else{
  //   return false
  // }
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  const replaces = str.replace(regexp, newWord)
  return replaces
}
